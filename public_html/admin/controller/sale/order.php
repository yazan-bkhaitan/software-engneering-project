...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
        ...
        $data['oc_pom_status']              = $this->config->get('module_oc_pom_status');

        $data['column_po_status']           = $this->language->get('column_po_status');

        $data['button_view_po']             = $this->language->get('button_view_po');

        $data['button_generate_po']         = $this->language->get('button_generate_po');

        $data['button_generate_bulk_po']    = $this->language->get('button_generate_bulk_po');

        $data['error_select_order']         = $this->language->get('error_select_order');

        if(isset($this->session->data['success'])){

            $data['po_success']  = $this->session->data['success'];
		...
		foreach ($results as $result) {
            ...
            $po_status  = $this->language->get('text_not_available');

            $po_id 	= $po_link = false;

            if($this->config->get('module_oc_pom_status')){

                $getPO = $this->model_po_mgmt_po_order->getPOEntry($result['order_id']);

                if(!empty($getPO) && $getPO['id']){

                    $po_link = $this->url->link('po_mgmt/po_order', 'user_token=' . $this->session->data['user_token'] . '&filter_order_id='. $result['order_id'],true);

                    $po_status = $this->language->get('text_po_available');
	...
	public function getForm() {
        ...
        if(isset($this->request->get['po_mgmt']) && $this->request->get['po_mgmt'] == true) {
	...
	public function info() {
		...
		if ($order_info) {
            ...
            $data['date_added_details'] = $order_info['date_added'];
			...
			foreach ($products as $product) {
                ...
                $added_to_po = $this->model_sale_order->checkProductAddedToPO($this->request->get['order_id'], $product['product_id']) ? true : false;

				$data['products'][] = array(
					'order_product_id' => $product['order_product_id'],
					'product_id'       => $product['product_id'],
					'name'    	 	   => $product['name'],
                    'added_to_po'	   => $added_to_po,
					'model'    		   => $product['model'],
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
    ...
    public function addToPO(){
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['product_id']) && isset($this->request->post['order_id']) && isset($this->request->post['quantity'])) {
            $this->load->model('sale/order');
            $this->load->model('catalog/product');
            $this->load->model('setting/setting');
            ...
            if ($product_details) {
                ...
                if ($supplier_id) {
                    ...
                    $ordered_qty = $this->request->post['quantity'];
                    $purchase_date = $this->request->post['purchase_date'];
                    $ordered_cost = $product_details['cost_price'];
                    $sub_total = $ordered_cost * $ordered_qty;
                    $tax = isset($moduleConfigSetting['module_oc_pom_tax_cost']) ? $moduleConfigSetting['module_oc_pom_tax_cost'] : 0;
                    $opended_po = $this->model_sale_order->getOpenSupplierQuotaion($supplier_id);
                    if ($opended_po) {
                        $po_id = $opended_po['id'];
                        $prdouct_in_po = $this->model_sale_order->getPOProduct($po_id, $product_id);
                        if($prdouct_in_po){
                            $pd_po_qty = $prdouct_in_po['ordered_qty'];
                            $pd_po_sub_total = $prdouct_in_po['sub_total'];
                            $pd_po_sub_total += $sub_total;
                            $pd_po_qty += $ordered_qty;
                            $this->model_sale_order->updateOPProductQty($po_id, $product_id, $pd_po_qty, $pd_po_sub_total);
                            $this->model_sale_order->addOrderIdToOPProduct($po_id, $order_id, $product_id);
                        }else {
                            $this->model_sale_order->addToPO($po_id, $supplier_id, $order_id, $product_id, $ordered_cost, $ordered_qty, $sub_total, $tax);
                        }
                        $total = $this->model_sale_order->getPOTotal($po_id);
                        $total += $sub_total;
                        $this->model_sale_order->updatePOTotal($po_id, $total);
                    } else {
                        $shipping_address = '';
                        $shipping_method_id = 0;
                        $shipping_method = 'N/A';
                        $shipping_cost = '0.0000';
                        $shipping_start_date = '0000-00-00 00:00:00';
                        $shipping_expected_date = '0000-00-00 00:00:00';
                        $payment_term = 'N/A';
                        $payment_term_id = 0;
                        $order_source = '';
                        $total = $sub_total;
                        $current_datetime = date('Y-m-d H:i:s', time());
                        $po_id = $this->model_sale_order->addPO($supplier_id, $shipping_address, $shipping_method_id, $shipping_method, $shipping_cost, $shipping_start_date, $shipping_expected_date, $payment_term_id, $payment_term, $order_source, $total, $current_datetime, $purchase_date);
                        $this->model_sale_order->addToPO($po_id, $supplier_id, $order_id, $product_id, $ordered_cost, $ordered_qty, $sub_total, $tax);
                    }
                    $json['success'] = 'Success: Product added to the PO successfully!';
                } else {
                    $json['error'] = 'Supplier does not exist';
                }
            } else {
